...
SHEET_JSON = "service_account.json"  # Siz saqlagan Service Account JSON
SHEET_NAME = "Oylik test natijalari 2026 by telegram bot"  # Google Sheets nomi
...
try:
    ...
    print("Google Sheetsga ulanayotganda xatolik:", e)
    sheet = None
...
# =======================================
def save_to_sheets(ism, guruh, togri, jami, foiz):
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    if sheet:
        try:
            # Agar Sheet bo‘sh bo‘lsa, sarlavha qo‘shish
            if not sheet.get_all_values():
                sheet.append_row(["Guruh", "Ism", "To‘g‘ri", "Ball", "Foiz", "Vaqt"])
            sheet.append_row([guruh, ism, togri, togri, foiz, now])
        except Exception as e:
            print("Google Sheetsga yozishda xatolik:", e)

# =======================================
# EXCELGA SAQLASH (oldingi funksiya)
# =======================================
def save_to_excel_grouped(ism, guruh, togri, jami, foiz):
    filename = "Oylik test natijalari 2026 by telegram bot.xlsx"
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
async def tekshir(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Natijalarni Google Sheetsga va Excelga yozish
    save_to_sheets(
